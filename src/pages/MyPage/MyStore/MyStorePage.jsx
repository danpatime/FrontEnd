  ...
  useEffect(() => {
    const fetchStoreData = async () => {
      try {
        ...
        const response = await request.get(`/api/v1/business?businessId=${user.id}`);
        const data = response;
  
        // data가 배열인 경우 그대로 사용하고, 객체인 경우 배열로 변환
      ...
      <Page>
        ...
        <StoreList>
          {Array.isArray(storeData) && storeData.length > 0 ? (
            storeData.map((store) => (
              <StoreItem key={store.businessId}>
                ...
                <div>
                  ...
                  <div className="store-info">
                    <div>
                      <span>주소</span>
                      <p>{store.location.address} {store.location.detailAddress}</p>
                    </div>
                    <div>
                      <span>대표명</span>
